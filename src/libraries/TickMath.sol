...
/// @title TickMath — tick ↔ sqrtPriceX96 conversions
/// @notice Computes sqrt(1.0001^tick) as a Q64.96 fixed point number
/// @dev Tick range: [MIN_TICK, MAX_TICK], inclusive
library TickMath {
    ...
    /// @dev The minimum sqrtPriceX96 that corresponds to MIN_TICK
    uint160 internal constant MIN_SQRT_PRICE = 4295128739;
    /// @dev The maximum sqrtPriceX96 that corresponds to MAX_TICK
    uint160 internal constant MAX_SQRT_PRICE = 1461446703485210103287273052203988822378723970342;

    /// @notice Given a tick, calculates sqrt(1.0001^tick) * 2^96
    /// @dev Uses a lookup-table approach with 20 precomputed ratios
    ...
    /// @notice Given a sqrtPriceX96, calculates the greatest tick such that getSqrtPriceAtTick(tick) <= sqrtPriceX96
    /// @dev Uses msb and log2 approach
    function getTickAtSqrtPrice(uint160 sqrtPriceX96) internal pure returns (int24 tick) {
        unchecked {
            ...
            // Fine grained: compute log_2 * 2^64
